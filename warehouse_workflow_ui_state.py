...
def derive_workflow_ui_state(source: Mapping[str, Any]) -> dict[str, Any]:
    ...
    rules = data and bool(source.get("ruleset_valid")) and source.get("rule_dependencies_valid") is True
...
def state_from_session(model: Mapping[str, Any] | None, session: Mapping[str, Any]) -> dict[str, Any]:
    ...
    current = session.get("placement_comparison_baseline") or session.get("placement_comparison_current_state") or {}
    demand = session.get("placement_comparison_outbound_demand_state") or {}
    benchmark = session.get("placement_comparison_benchmark") or {}
    scenario = session.get("placement_comparison_scenario") or {}
    proposed = scenario.get("proposed_state") or benchmark.get("proposed_state") or session.get("placement_comparison_proposed_state") or {}
    ...
    return derive_workflow_ui_state({
        ...
        "start_ready": bool(current) and current.get("readiness", {}).get("opening_stock_business_ready") is True,
        "warehouse_selected": bool(session.get("outbound_selected_warehouse") or current.get("warehouse_key")),
        ...
        "mandatory_data_checks_passed": session.get("outbound_mandatory_data_checks_passed") is True,
        "ruleset_valid": bool(session.get("workspace_rule_config")),
        "rule_dependencies_valid": session.get("workspace_rule_dependencies_valid") is True,
        "proposed_exists": bool(proposed),
        "proposed_stale": bool(session.get("placement_comparison_signature") and
                               session.get("placement_comparison_signature") != session.get("placement_comparison_active_signature")),
        "benchmark_exists": bool(comparison),
        "benchmark_stale": bool(session.get("placement_comparison_distance_signature") and
                                session.get("placement_comparison_distance_signature") != session.get("placement_comparison_active_distance_signature")),
        "gate_valid": bool((session.get("workspace_gate_state") or
                            session.get("placement_comparison_gate_state") or {}).get("gates")),
        "benchmark_prerequisites_ready": session.get("placement_comparison_benchmark_prerequisites_ready") is True,
    })
