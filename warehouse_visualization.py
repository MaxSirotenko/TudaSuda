...
def build_virtual_warehouse_html(sheet, scale=34):
    ...
    for value in sheet.values:
        position = (value["column"], value["row"])
        if position in occupied_cells:
            continue
        label = html.escape(value["value"])
        left = value["column"] * scale
        top = value["row"] * scale
        parts.append(
            f"<div title='{label}' style='position:absolute;left:{left}px;top:{top}px;"
            f"max-width:{scale * 4}px;height:{scale - 4}px;color:#f59e0b;"
            f"font:12px Arial;line-height:{scale - 4}px;white-space:nowrap;"
            f"overflow:hidden;text-overflow:ellipsis;pointer-events:none'>{label}</div>"
        )
    for row in sheet.rows:
