...
def test_characteristics_and_unit_variants_are_separate_and_sorted_by_source():
    ...
    assert [(item["characteristic_name"], item["unit_name"]) for item in demands] == [("Белая", ""), ("Красная", "короб"), ("Белая", "штука")]
    assert result["diagnostics"]["unit_variants_for_same_sku"] == 1


def test_existing_keys_are_preserved_and_missing_keys_use_canonical_helpers():
    ...
    assert result["orders"][0]["demands"][0]["sku_key"] == "custom-sku"
...
def test_demand_key_is_stable_quantity_independent_and_collision_safe():
    ...
    assert json.loads(first) == ["a,b", "c", "короб"]
