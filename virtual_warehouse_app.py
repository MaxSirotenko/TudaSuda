...
from warehouse_outbound_orders import (
    build_outbound_tooltip_context,
...
def _build_geometry_dynamic_layer_direct(
    ...
    outbound_context = build_outbound_tooltip_context(model, placement_state)
    return build_geometry_dynamic_payload_from_state(
        model, placement_state, snapshot, render_settings, outbound_context,
    )


@st.cache_data(show_spinner=False)
def build_geometry_dynamic_layer_cached(
    _model: dict, _placement_state: dict, dynamic_revision_token: tuple,
    _render_settings: dict, renderer_version: int = GEOMETRY_DYNAMIC_CACHE_VERSION,
) -> dict:
    return _build_geometry_dynamic_layer_direct(_model, _placement_state, _render_settings)
...
def render_geometry_map_view(model: dict) -> None:
    ...
    dynamic_placement_state = placement_state if not placement_warning else {}
    ...
    with measure_step("build_geometry_dynamic_layer", dynamic_metadata):
        if dynamic_token is None:
            dynamic_payload = _build_geometry_dynamic_layer_direct(model, dynamic_placement_state, label_settings)
        else:
            dynamic_payload = build_geometry_dynamic_layer_cached(
                model, dynamic_placement_state, dynamic_token, label_settings, GEOMETRY_DYNAMIC_CACHE_VERSION,
            )
    ...
    st.session_state["geometry_layer_diagnostics"] = {
        ...
        "dynamic_builder_mode": "direct_state", "placements_count": len(dynamic_placement_state.get("placements", [])),
        "interesting_cell_count": len(dynamic_payload), "full_model_enrichment": "нет", "full_model_deepcopy": "нет",
    }
