...
## Обновление локальной папки после merge PR

Merge PR в GitHub не обновляет автоматически файлы в локальной папке Windows.
`start.cmd` перед запуском пытается безопасно обновить текущую ветку через
`git fetch --prune` и `git pull --ff-only`. Автообновление выполняется только если
рабочая папка git чистая. Если есть локальные изменения или конфликты, `start.cmd`
не будет их затирать и напишет причину в `start.log`.

Если нужно обновиться вручную:
