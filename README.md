...
- ручные добавления, изменения и удаления ячеек выполняются поверх текущей модели, не меняют исходный Excel и сохраняются в `data/last_import/manual_overrides.json`;
- при загрузке и построении нового Excel старые ручные изменения сбрасываются, чтобы не применяться к другой выгрузке;
- текущую модель с учётом ручных правок можно скачать из интерфейса в `current_warehouse_model.xlsx`.
