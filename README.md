...
- для рядов можно задать тип `Обычный ряд` или `Набивной ряд`; набивной ряд хранит одну системную ячейку как широкий логический блок с 2–7 физическими паллетоместами;
- настройки набивных рядов сохраняются в модели и в `data/last_import/row_settings.json`, а при построении нового Excel сбрасываются вместе с ручными правками;
- экспорт текущей модели содержит поля вместимости: `storage_type`, `deep_lane_width`, `capacity_pallets`, `volume_m3`, `cell_direction`.
