...
WAREHOUSE_ZONE_ALIASES = ["warehouse_zone", "зона", "зона склада"]
...
def normalize_receipt_table(df: pd.DataFrame, mapping: dict[str, str | None]) -> tuple[pd.DataFrame, dict[str, Any], list[dict[str, str]]]:
    ...
    if not any(mapping.get(key) for key in ["qty_pallets", "qty_boxes", "qty_units"]):
        messages.append({"level": "error", "message": "Не выбрана ни одна колонка количества: паллеты, короба или базовое количество."})
